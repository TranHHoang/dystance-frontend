...
const whiteboard = {
  ...
  getRoundedAngles: function (currentPos) {
    ...
    const outX = startCoords.x + dist * Math.cos(angle45);
    const outY = startCoords.y - dist * Math.sin(angle45);
  ...
  triggerMouseOver: function () {
    ...
    if (!_this.mouseover) {
      ...
      let widthHeight = _this.thickness;
      if (_this.tool === "eraser") {
        color = "#00000000";
  ...
  delKeyAction: function () {
    ...
    $.each(_this.mouseOverlay.find(".dragOutOverlay"), function () {
      ...
      const top = Math.round(p.top * 100) / 100;
      _this.drawId++;
  ...
  drawPenSmoothLine: function (coords, color, thickness) {
    ...
    const x2 = coords[6];
    const y2 = coords[7];
    const length = Math.sqrt(Math.pow(x0 - x1, 2) + Math.pow(y0 - y1, 2));
    const steps = Math.ceil(length / 5);
    _this.ctx.beginPath();
    _this.ctx.moveTo(x0, y0);
    for (let i = 0; i < steps; i++) {
      const point = lanczosInterpolate(xm1, ym1, x0, y0, x1, y1, x2, y2, (i + 1) / steps);
  ...
  addTextBox(textcolor, fontsize, left, top, txId, newLocalBox) {
    ...
    textBox.on("mousemove touchmove", function (e) {
      ...
      let currX = e.offsetX + textBoxPosition.left;
      const currY = e.offsetY + textBoxPosition.top;
    ...
    textBox.find(".textContent").on("input", function () {
      const text = btoa(unescape(encodeURIComponent($(this).html()))); //Get html and make encode base64 also take care of the charset
  ...
  undoWhiteboard: function (username) {
    ...
    for (var i = _this.drawBuffer.length - 1; i >= 0; i--) {
      if (_this.drawBuffer[i]["username"] == username) {
        const drawId = _this.drawBuffer[i]["drawId"];
        for (var i = _this.drawBuffer.length - 1; i >= 0; i--) {
  ...
  redoWhiteboard: function (username) {
    ...
    for (var i = _this.undoBuffer.length - 1; i >= 0; i--) {
      if (_this.undoBuffer[i]["username"] == username) {
        const drawId = _this.undoBuffer[i]["drawId"];
        for (var i = _this.undoBuffer.length - 1; i >= 0; i--) {
  ...
  handleEventsAndData: function (content, isNewData, doneCallback) {
    ...
    const color = content["c"];
    const username = content["username"];
    const thickness = content["th"];
    window.requestAnimationFrame(function () {
      if (tool === "line" || tool === "pen") {
        ...
        if (content["draw"] == "1") {
          _this.drawImgToCanvas(content["url"], data[0], data[1], data[2], data[3], data[4], doneCallback);
        } else {
  ...
  getImageDataBase64(options, callback) {
    const _this = this;
    const width = this.mouseOverlay.width();
    const height = this.mouseOverlay.height();
    ...
    const destCtx = copyCanvas.getContext("2d"); //Draw the maincanvas to the exportcanvas
    ...
    $.each($(".textBox"), function () {
      ...
      const p = textContainer.position();
    ...
    function checkForReturn() {
      if (textBoxCnt == 0) {
        const url = copyCanvas.toDataURL("image/" + imageFormat);
        callback(url);
      }
    }
    checkForReturn();
...
function lanczosInterpolate(xm1, ym1, x0, y0, x1, y1, x2, y2, a) {
  let cm1 = lanczosKernel(1 + a);
  let c0 = lanczosKernel(a);
  let c1 = lanczosKernel(1 - a);
  let c2 = lanczosKernel(2 - a);
  const delta = (cm1 + c0 + c1 + c2 - 1) / 4;
  cm1 -= delta;
  c0 -= delta;
  c1 -= delta;
  c2 -= delta;
